#!/usr/bin/env perl
#original script by Douglas G Schofield
#email : douglasgscofield@gmail.com
#github : https://github.com/douglasgscofield/bioinfo
use warnings;
use strict;
use Getopt::Long;
use Bio::Seq;
...
usage("") if $o_help;
#only fixed up the SNP, do not care for INDEL at the moment
#usage("--type must be SNP or INDEL") if $o_type and $o_type ne "INDEL" and $o_type ne "SNP";
usage("--type must be SNP ") if $o_type and  $o_type ne "SNP";
